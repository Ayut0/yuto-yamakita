...
# 要約
80&Company にインターンとして入社後、UX の向上やシステムパフォーマンスの最適化、国際化対応の実装を担当。20 個以上の再利用可能なフロントエンドコンポーネントを実装および Web サイトのパフォーマンス向上に貢献。その後、Fresh Tracks Canada に入社し A / Bテストを通じて予約件数を 40% 増加させる。
後にフリーランスエンジニアに転身し株式会社Penetratorに入社。主にフロントエンドエンジニアとして新規機能の開発等を行いながら、バックエンド開発にも従事。

エンジニアのみならずデザイナー、QA、プロダクトマネージャーとコミュニケーションを取りながら円滑に業務を進めることを得意とする。
